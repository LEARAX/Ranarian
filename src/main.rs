...
impl EventHandler for Handler {
    fn on_message(&self, ctx: Context, msg: Message) {
        ...
        if &msg.content[..5] == "!play" {
            ...
            match ctx.shard.lock().manager.get(self.target_voice_channel.0) {
                Some(handler) => match voice::ytdl(url) {
                    Ok(source) => {
    ...
    fn on_ready(&self, ctx: Context, ready: Ready) {
        ...
        shard
            .manager
            .join(self.target_voice_channel.0, self.target_voice_channel.1);
    }
}

fn main() {
    let token = env::var("DISCORD_TOKEN").expect("Expected a token in the environment");
    let target_voice_guild =
        env::var("TARGET_VOICE_GUILD").expect("Expected a target voice guild ID");
    let target_voice_channel =
        env::var("TARGET_VOICE_CHANNEL").expect("Expected a target voice channel ID");
        ...
        Handler {
            target_voice_channel: (
                GuildId(
                    target_voice_guild
                        .parse::<u64>()
                        .expect("Expected guild ID to be numeric"),
                ),
                ChannelId(
                    target_voice_channel
                        .parse::<u64>()
                        .expect("Expected channel ID to be numeric"),
                ),
            ),
        },
    );
